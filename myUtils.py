...
from GIF import fusion_loss_mef
...
#         self.crop_resize = (256,256)
#         self.mode = mode
#         if mode == "train":
#             self.srcA = os.path.join(args.SERVER.TRAIN_DATA, "trainA")  # 欠曝图像路径
#             self.srcB = os.path.join(args.SERVER.TRAIN_DATA, "trainB")  # 过曝图像路径
#             self.files = sorted(os.listdir(self.srcA))[:args.SERVER.train_len]
#         elif mode == "val":
#             self.srcA = os.path.join(args.SERVER.VAL_DATA, "trainA")
#             self.srcB = os.path.join(args.SERVER.VAL_DATA, "trainB")
#             self.files = sorted(os.listdir(self.srcA))[:args.SERVER.val_len]
#         elif mode == "test":
#             self.srcA = os.path.join(args.SERVER.TEST_DATA, "under")
#             self.srcB = os.path.join(args.SERVER.TEST_DATA, "over")
#             self.files = sorted(os.listdir(self.srcA))[:args.SERVER.test_len]
...
#     def ImgRead(self, fileA, fileB, hflip, vflip, cropIndex):
#         # 忽略大小写查找文件路径
#         fileA = find_file_ignore_case(self.srcA, os.path.basename(fileA))
#         fileB = find_file_ignore_case(self.srcB, os.path.basename(fileB))

#         # 读取图像文件
#         img0 = cv2.imread(fileA, -1)
#         img1 = cv2.imread(fileB, -1)
...
#         # 数据增强
#         if self.mode == "train":
#             img0 = self.img_aug(img0, hflip, vflip, cropIndex, crop_resize=self.crop_resize)
#             img1 = self.img_aug(img1, hflip, vflip, cropIndex, crop_resize=self.crop_resize)
#         elif self.mode == "val":
#             img0 = img0.resize((256, 256))
#             img1 = img1.resize((256, 256))

#         # 转换为 numpy 数组
#         img0 = np.array(img0, dtype=np.uint8).astype(np.float32)
#         img1 = np.array(img1, dtype=np.uint8).astype(np.float32)

#         return img0, img1

#     def __getitem__(self, index):
#         name = self.files[index]
#         fileA = os.path.join(self.srcA, name)  # 欠曝图像路径
#         fileB = os.path.join(self.srcB, name)  # 过曝图像路径

#         # Prepare for augmentation
#         temp = cv2.imread(fileA)
#         temp_img = Image.fromarray(cv2.cvtColor(temp, cv2.COLOR_BGR2RGB))
#         (w, h) = temp_img.size
#         hflip, vflip, cropIndex = self.random_prob(h=h, w=w)

#         # Read and augment images
#         img0, img1 = self.ImgRead(fileA, fileB, hflip, vflip, cropIndex)

#         img0_Y, img0_Cb, img0_Cr = RGB2YCbCr_np(img0)
#         img1_Y, img1_Cb, img1_Cr = RGB2YCbCr_np(img1)

#         # Convert images and gradients to tensors
...
#         # Convert to YCbCr and calculate gradients
#         img0_Y, img0_Cb, img0_Cr = RGB2YCbCr_np(img0)
#         img1_Y, img1_Cb, img1_Cr = RGB2YCbCr_np(img1)
...
class My_Dataset(Dataset):
    def __init__(self, args, mode='train'):
        ...
        # 根据模式设置数据路径
        if mode == "train":
            self.data_dir = args.SERVER.TRAIN_DATA
            self.max_len = args.SERVER.train_len
        elif mode == "val":
            self.data_dir = args.SERVER.VAL_DATA
            self.max_len = args.SERVER.val_len
        elif mode == "test":
            self.data_dir = args.SERVER.TEST_DATA
            self.max_len = args.SERVER.test_len
        
        # 获取所有子文件夹（每个子文件夹代表一对图片）
        self.folders = [f for f in os.listdir(self.data_dir) if os.path.isdir(os.path.join(self.data_dir, f))]
        self.folders.sort()
        
        # 限制处理的图片数量
        if self.max_len is not None and self.max_len > 0:
            self.folders = self.folders[:self.max_len]
    ...
    def ImgRead(self, folder_path, hflip, vflip, cropIndex):
        # 查找文件夹中的 _A 和 _B 图片
        ...
        for file in files:
            if file.endswith('_A.png') or file.endswith('_A.jpg') or file.endswith('_A.jpeg'):
                img_A_file = os.path.join(folder_path, file)
            elif file.endswith('_B.png') or file.endswith('_B.jpg') or file.endswith('_B.jpeg'):
                img_B_file = os.path.join(folder_path, file)
        
        if img_A_file is None or img_B_file is None:
            raise FileNotFoundError(f"无法在文件夹 {folder_path} 中找到 _A 和 _B 图片对")

        # 读取图像文件
        img0 = cv2.imread(img_A_file, -1)
        img1 = cv2.imread(img_B_file, -1)
    ...
    def __getitem__(self, index):
        ...
        # 获取文件夹中的任意一个图片来确定尺寸
        files = os.listdir(folder_path)
        sample_img_path = os.path.join(folder_path, files[0])
        temp = cv2.imread(sample_img_path)
        temp_img = Image.fromarray(cv2.cvtColor(temp, cv2.COLOR_BGR2RGB))
        (w, h) = temp_img.size
        hflip, vflip, cropIndex = self.random_prob(h=h, w=w)

        # 读取和增强图像
        img0, img1 = self.ImgRead(folder_path, hflip, vflip, cropIndex)
        ...
        # 返回样本和元数据
        # 使用文件夹名称作为图片名称
        return samples, folder_name, "png"  # 默认使用png作为输出格式
...
def train_one_epoch(args, model, exposure_correction_model, dataloader_train, optimizer, lr_scheduler, epoch, logger, scaler):
    model.train()
    exposure_correction_model.train()
    ...
    start = time.time()
    totalloss = 0
    for freq, samples in enumerate(dataloader_train):
        ...
        if args.TRAIN.loss == "GIF":
            loss_g = gra_Loss(fus_Y, img0_Y, img1_Y)
            loss = loss_g
        #----------------------------------------------------------------------
        optimizer.zero_grad()
        loss.backward()
        ...
        if freq % args.TRAIN.PRINT_FREQ == 0 and (args.distributed == False or torch.distributed.get_rank() == 0):
            logger.info(f"||train -- epoch:{epoch} / {args.TRAIN.EPOCHS} --step:{freq+1} / {num_steps} --lr:{cur_lr:.7f}-----curloss: {loss.item():.5f}")
...
@torch.no_grad()
def validate(args, model, exposure_correction_model, dataloader_val, epoch, logger, dataset_val_len):
    model.eval()
    ...
    start = time.time()
    totalloss = 0
    for freq, samples in enumerate(dataloader_val):
        if freq % args.TRAIN.PRINT_FREQ == 0 and (args.distributed == False or torch.distributed.get_rank() == 0):
            logger.info(f"|| Val -- epoch:{epoch} / {args.TRAIN.EPOCHS} ------step:{freq+1} / {num_steps}")
        img0_RGB = samples[0]['img0_RGB'].cuda()
        img0_Y = samples[0]['img0_Y'].cuda()
        img0_Cb = samples[0]['img0_Cb'].cuda()
        img0_Cr = samples[0]['img0_Cr'].cuda()
        ...
        #     loss = loss_pmgi
        fus_Cb = colorCombine_torch([img0_Cb*255, img1_Cb*255]) #[B C H W], 255
        fus_Cr = colorCombine_torch([img0_Cr*255, img1_Cr*255])
        fus_RGB = YCbCr2RGB_torch(fus_Y*255, fus_Cb, fus_Cr)      #B C H W, 255
...
def setup_logger(logger_name, root, level=logging.INFO, screen=False, tofile=True):
    ...
    lg.setLevel(level)
    if tofile:
        log_file = os.path.join(root,""+'MEF_{}.log'.format(get_timestamp()))
