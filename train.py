...
def run(args):
    ...
    exposure_correction_model.load_state_dict(torch.load("/nvme0/shared_data/MambaMEF/model/best_Epoch_exposure.pth"))
    model.cuda(local_rank)
    exposure_correction_model.cuda(local_rank)
    ...
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.TRAIN.EPOCHS, eta_min=0, last_epoch=-1, verbose=False)
    # ---------------------------------dataloader--------------------------------
    ...
    test_Cb = [torch.randn(2,1,256,256).cuda(), torch.randn(2,1,224,224).cuda()]
    test_Cr = [torch.randn(2,1,256,256).cuda(), torch.randn(2,1,224,224).cuda()]
    test_gra = [torch.randn(2,1,256,256).cuda(), torch.randn(2,1,224,224).cuda()]
    model.eval()
    ...
    for epoch in range(start_epoch, args.TRAIN.EPOCHS):
        #train
        train_one_epoch(args, model, exposure_correction_model, dataloader_train, optimizer, lr_scheduler, epoch, logger, scaler)
        #validation
        eval_status = validate(args, model, exposure_correction_model, dataloader_val, epoch, logger, dataset_val_len)
